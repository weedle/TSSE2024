...
{
    private ShipDefinitions.State state = ShipDefinitions.State.Searching;
    ...
    {
        if (state == ShipDefinitions.State.Searching)
        {
            ...
            */
            ...
            {
                this.target = obj;
                state = ShipDefinitions.State.Aiming;
            }
            else
            {
                ship.brake();
        ...
        else if (state == ShipDefinitions.State.Aiming)
        {
            bool move = true;
            if (target == null)
            {
                state = ShipDefinitions.State.Searching;
                return;
            }
                ...
                shipAngle - shipObject.getEffectiveAngle() < targetAngle))
            {
                        ...
                        target.transform.position) < shipObject.getEffectiveDistance())
                {
                    ...
                    if (target.GetComponent<ShipController>()
                            .getFaction() != faction)
                            state = ShipDefinitions.State.Firing;
                        else
                        {
                            ship.brake();
                            move = false;
        ...
        /*
        else if (state == ShipDefinitions.State.Firing)
        {
            ship.fire();

            if (GetComponent<FiringModule>().canFire() == false)
            {
                state = ShipDefinitions.State.Cooling;
            }
        }
        else if (state == ShipDefinitions.State.Cooling)
        {
    ...
    public ShipDefinitions.State getState()
    {
        return state;
    }
